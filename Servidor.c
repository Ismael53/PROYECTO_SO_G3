...
int PonJugadorPartida (Partida *partida,char nombre[20],int socket){ //retorna -1 si la partida ya esta llena (contiene 5 jugadores) rotorna 0 si el jugador se puede a￱adir a la partida y retorna -2 si no encuentra el jugador en la lista de conectados
	int encontrado=0;
	int i=0;
	if (partida->num==5)
		return -1;
...
int PonPartida(partidas Tabla,Partida partida) //funcion para insertar una partida en la tabla de partidas; retorna 0 si la partida se puede a￱adir correctamente, retorna -1 si no ha podido a￱adir el 
{
	int encontrado;
	int i=0;
...
void DamePartidas(partidas tabla,char partidas[1000],char usuario[20])// funcion para retornar las partidas disponibles de un jugador
	// escribe notificacion 11/num partidas/id partida_1/juego_partida1/id_partida2/juego_partida_2
{
	int i;
	int contador=0;
...
int Eliminar (ListaConectados *lista,char nombre[20]) //esta funci￯﾿ﾳn retorna -1 si no encuentra el nombre que se debe eliminar de la lista y 0 si se ha eliminado correctamente
{
...
int signIn(MYSQL *conn ,char respuesta[1000],char nombre [20],char contra [20])
{
	...
	char consulta [512];
	char consulta1[512];
	sprintf (consulta,"SELECT JUGADORES.NOMBRE FROM (JUGADORES) WHERE JUGADORES.NOMBRE = BINARY '%s' ",nombre); 
...
void Consulta_2(MYSQL *conn ,char respuesta[1000])
{
	...
	if (row == NULL){
		...
		printf("La consulta retorna el jugador o los jugadores con el mayor dep￯﾿ﾳsito y el menor n￯﾿ﾺmero de partidas ganadas:\n");
	while (row !=NULL) {
		// la columna 0 contiene el nombre del jugador
...
void Consulta_3( MYSQL *conn, char respuesta[1000])
{
	...
	//consulta adicional para saber la ID de la partida que en la que gan￯﾿ﾯ￯ﾾ﾿￯ﾾﾯ￯﾿ﾯ￯ﾾﾾ￯ﾾ﾿￯﾿ﾯ￯ﾾﾾ￯ﾾﾯ￯﾿ﾯ￯ﾾ﾿￯ﾾﾯ￯﾿ﾯ￯ﾾﾾ￯ﾾﾾ￯﾿ﾯ￯ﾾﾾ￯ﾾ﾿￯﾿ﾯ￯ﾾ﾿￯ﾾﾯ￯﾿ﾯ￯ﾾﾾ￯ﾾﾾ￯﾿ﾯ￯ﾾﾾ￯ﾾﾳ el jugador que se busca en la consulta anterior
	else
	{
		strcpy(respuesta,"5/");
...
void *AtenderCliente(void *socket)
{
	...
	while (terminar ==0)
	{
		...
		else if (codigo==8) //peticion de aceptar o declinar una invitacion de partida
		{
			...
			if (pos!=-1)
			{
				...
				else
				{
					...
					write (sock_inv,notificacion, strlen(notificacion));
		...
		else if (codigo==9)//peticion de enviar por el chat 9/codigo partida/autor/mensaje
		{
			p = strtok( NULL, "/");
			int Juego;
			Juego= atoi(p);
			...
			int pos= DamePosicionPartidaID(miPartidas,IDpartida);
			for (int j=0;j<miPartidas[pos].num;j++)
			{
		...
		int j;
		if (codigo==0 || codigo ==2||introC==1||codigo==11)
		{
