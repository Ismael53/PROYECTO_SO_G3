...
namespace WindowsFormsApplication1
{
    ...
    public class Hand
    {
        ...
        List<Card> QuickSortSuit(List<Card> myCards)
        {
            ...
            {
                if (myCards[i].gettipo() > pivot.gettipo())
                {
                    greater.Add(myCards[i]);
                }
                else if (myCards[i].gettipo() <= pivot.gettipo())
                {
                    less.Add(myCards[i]);
                }
            }
        ...
        public override string ToString()
        {
            ...
            {
                case 1:
                    return Card.numeroToString(handValue[1]) + " High";
                case 2:
                    return "Pair of " + Card.numeroToString(handValue[1]) + "s";
                case 3:
                    return "Two Pair: "+Card.numeroToString(handValue[1]) + "s over " + Card.numeroToString(handValue[2])+"s";
                case 4:
                    return "Three " + Card.numeroToString(handValue[1]) + "s";
                case 5:
                    return Card.numeroToString(handValue[1]) + " High Straight";
                case 6:
                    return Card.numeroToString(handValue[1]) + " High Flush";
                case 7:
                    return Card.numeroToString(handValue[1]) + "s Full of " + Card.numeroToString(handValue[2]) + "s";
                case 8:
                    return "Quad " + Card.numeroToString(handValue[1]) + "s";
                case 9:
                    return Card.numeroToString(handValue[1]) + " High Straight Flush";
                default:
                    return "Royal Flush";
            }
        }
        ...
        public bool isEqual(Hand a)
        {
            ...
            {
                if (a[i] != myHand[i] || a[i].gettipo() != myHand[i].gettipo())
                    return false;
            }
            return true;
        }
